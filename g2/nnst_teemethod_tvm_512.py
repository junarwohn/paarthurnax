...
def new_data_cb(sink, buffer):
    ...
    if running:
        ...
        if result:
           ...
           g_tensor_result = np.reshape(content_arr, (512, 512)) 
...
pipeline_str = \
    ...
    t_raw. ! queue ! videoconvert ! cairooverlay name=tensor_res ! ximagesink name=img_tensor \
    t_raw. ! queue leaky=2 max-size-buffers=2 ! videoscale ! video/x-raw,width=512,height=512! \
    tensor_converter ! \
    tensor_transform mode=arithmetic option=typecast:float32,div:255 ! \
    tensor_transform mode=dimchg option=0:2 ! \
    tensor_filter framework=tvm model=/home/j/paarthurnax/g2/unet_tvm_cpu_512.so input=512:512:3 inputtype=float32 custom=device:CPU,num_input_tensors:1 ! \
    tensor_sink name=tensor_sink'
